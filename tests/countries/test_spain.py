...
def test_spain_valid(vat):
    result = check_vat(vat)
    assert result.is_valid, f"Expected {vat} to be valid for Spain"
    assert result.country.name == "Spain"


@pytest.mark.parametrize("vat", INVALID_VATS)
def test_spain_invalid(vat):
    result = check_vat(vat)
    ...
    assert not result.is_valid, f"Expected {vat} to be invalid for Spain"
