...
def test_france_valid(vat):
    result = check_vat(vat)
    assert result.is_valid, f"Expected {vat} to be valid for France"
    assert result.country.name == "France"


@pytest.mark.parametrize("vat", INVALID_VATS)
def test_france_invalid(vat):
    result = check_vat(vat)
    ...
    assert not result.is_valid, f"Expected {vat} to be invalid for France"
