...
def test_belgium_valid(vat):
    result = check_vat(vat)
    assert result.is_valid, f"Expected {vat} to be valid for Belgium"
    assert result.country.name == "Belgium"


@pytest.mark.parametrize("vat", INVALID_VATS)
def test_belgium_invalid(vat):
    result = check_vat(vat)
    ...
    assert not result.is_valid, f"Expected {vat} to be invalid for Belgium"
