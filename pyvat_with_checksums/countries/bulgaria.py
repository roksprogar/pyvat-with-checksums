...
def _increase2(value: int, vat: str, start: int, end: int, multipliers: List[int]) -> int:
    result = value
    for i in range(start, end):
...
def calc_bulgaria(vat: str) -> bool:
    ...
    return (
        _is_physical_person(vat, multipliers['physical']) or
        _is_foreigner(vat, multipliers['foreigner']) or
        _miscellaneous_vat(vat, multipliers['miscellaneous'])
    )
