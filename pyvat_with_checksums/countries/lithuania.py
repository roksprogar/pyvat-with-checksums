...
def _double_check_calculation_12(
    vat: str, total: int, multipliers_alt: List[int]
) -> int:
    result = total
    if total % 11 == 10:
        result = 0
...
def _check_12_digit_vat(
    vat: str, multipliers_med: List[int], multipliers_alt: List[int]
) -> bool:
    if len(vat) == 12:
...
def calc_lithuania(vat: str) -> bool:
    ...
    return _check_9_digit_vat(vat, multipliers["short"]) or _check_12_digit_vat(
        vat, multipliers["med"], multipliers["alt"]
    )
